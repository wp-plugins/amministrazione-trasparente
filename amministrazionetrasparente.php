...
function at_archive_buttons_pasw2015() { //Questa funzione va chiamata con at_archive_buttons()
include(plugin_dir_path(__FILE__) . 'shortcodes/shortcodes-php-archive-pasw2015.php');
}
...
function check_new_version($return) {
    ...
    if ( get_option('at_version_number') < $nuova_versione ) {
        if (get_option('at_version_number') == '') { require(plugin_dir_path(__FILE__) . 'taxonomygenerator.php'); }
        if (get_option('at_option_love')) { update_option( 'wpgov_show_love', '1' ); delete_option('at_option_love'); }
