...
function wpatt_reg_settings()
    ...
    if (get_option('wpatt_option_localization') == '')
        ...
        update_option('wpatt_option_localization', 'Allegati');
...
function wpatt_plugin_menu()
    {
    /* CODICE CAMBIATO DALLA VERSIONE ORIGINALE */
	add_submenu_page('edit.php?post_type=amm-trasparente', 'WP Attachments - Impostazioni', 'WP Attachments', 'manage_options', 'wpatt-option-page', 'wpatt_plugin_options');
    }
...
function wpatt_plugin_options()
    ...
    echo '<h2>WP Attachments 3.1 // Versione Speciale tradotta in italiano</h2>';
	echo '<h3>OPTIONS</h3>';
	...
	<table class="form-table">
        ...
        <th scope="row">Titolo dell\'elenco</th>

        <td><input type="text" name="wpatt_option_localization_n" value="';
    
    echo get_option('wpatt_option_localization');
    
    echo '" />&nbsp;Inserisci il titolo per la lista di allegati. Es. "<b>Allegati</b>"</td></tr>';
    
        echo '<tr><th scope="row">Includere le immagini?</th>
        <td><input type="checkbox" name="wpatt_option_includeimages_n" ';
    $wpatt_option_includeimages_get = get_option('wpatt_option_includeimages');
    if ($wpatt_option_includeimages_get == '1') {
		echo 'checked=\'checked\'';
	}
    echo '/>&nbsp;Spunta questa casella se vuoi che le immagini(.jpg, .jpeg, .gif, .png) vengano incluse nella lista.</td>';
    echo '</tr>';

	
    echo '<tr><th scope="row">Mostrare la data?</th>
        <td><input type="checkbox" name="wpatt_option_showdate_n" ';
    $wpatt_option_showdate_get = get_option('wpatt_option_showdate');
    if ($wpatt_option_showdate_get == '1') {
		echo 'checked=\'checked\'';
	}
    echo '/>&nbsp;Spunta questa casella se vuoi mostrare quando il file è stato caricato.</td>';
    echo '</tr></table>';
    
    
    
    echo '</table><p class="submit"><input type="submit" class="button-primary" name="Submit" value="Salva" /></p>';
    
    echo '</form></div>
	<h3>Esempio:</h3>
	<img class="screenshot" src="http://s-plugins.wordpress.org/wp-attachments/assets/screenshot-1.jpg?rev=761494">
	<h3>AIUTO, SUPPORTO & FEEDBACK</h3>
	<a href="http://wordpress.org/plugins/amministrazione-trasparente/" title "Amministrazione Trasparente Wordpress Plugin">http://wordpress.org/plugins/amministrazione-trasparente/</a><br/>
	...
	Grazie per utilizzare questo plugin.</div>';
